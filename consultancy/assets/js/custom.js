function sliderAutoPlay() {
		slide_duration	=	parseInt($("#slide_duration").val());
		setInterval(function(){ 
			var total_slides	=	parseInt($("#total_slides").val());
			var current_slide	=	parseInt($("#current_slide").val());
			var slide_direction	=	$("#slide_direction").val();
			if (slide_direction == 'rtl') {
				$('.next-container .icon-circle').trigger("click");
				current_slide	=	current_slide + 1;
				if (current_slide == total_slides) {
					$("#slide_direction").val("ltr");
				}
				$("#current_slide").val(current_slide);
			} else {
				current_slide	=	current_slide - 1;
				$('.prev-container .icon-circle').trigger("click");
				if (current_slide == 1) {
					$("#slide_direction").val("rtl");
					//current_slide = 1;
				}
				$("#current_slide").val(current_slide);
			}
...
function initMap() {
    ...
    if (null !== e) {
        if (null === t) {
            ...
            (n.type = "text/javascript"), (n.id = "map_api"), (n.src = ""), document.body.appendChild(n);
        }
        setTimeout(function () {
            try {
                var t = $("#map"),
    ...
    function n() {
        ...
        void 0 !== o ? (a = o) : a.top <= 70 && (a = s),
            r.getListener(function (e) {
                ...
                let o = 70;
                void 0 !== a && (o = a.top - 100), n > o ? (l < n ? t.addClass("nav-bg").addClass("hide-nave_REMOVE") : t.removeClass("hide-nave_REMOVE")) : t.removeClass("nav-bg").removeClass("hide-nave_REMOVE"), (l = n);
            });
    } 
    function a() {
